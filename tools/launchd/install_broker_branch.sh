...
# 20:00) tools/broker_branch_pull.py --no-crawl — i.e. INGEST ONLY.
#
# The model: the user prepares step 2 (the BSR crawl) earlier in the day,
# so by 20:00 the CSVs are already in twse_web/output/. This job just
...
print ""
print "──────────────────────────────────────────────────────────────"
print "ⓘ  Runs headless weekdays 20:00 → ingest only (--no-crawl)."
print "   YOU must prepare step 2 BEFORE 20:00: run the BSR crawler so"
