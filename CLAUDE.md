...
`_trading_day()` in [stock_web/app.py](stock_web/app.py) returns the current trading date, walking back over weekends to the prior Friday. **It rolls over at 16:30** (`TRADING_DAY_ROLLOVER_HOUR`/`_MINUTE`): before 16:30 local, today's TWSE data (daily OHLCV / T86 / 融資借券 publish in the afternoon, ~16:00–16:30) isn't fully out yet, so the "current trading day" stays on the previous completed day; at 16:30 it advances to today. The `refresh_watchlist` launchd job's afternoon run (16:30) fires right after this rollover to pull today's complete data. All daily caches (`{code}_{YYYYMMDD}.json`, `taiex_{YYYYMMDD}.json`, `t86_{YYYYMMDD}.json`, `companies_{YYYYMMDD}.json`) share this same tag. If today's TAIEX close hasn't been published yet (e.g. fetched intraday or right after 13:30 close), `taiex` rows come back null and the frontend's `taiexBar` ([stock_web/static/index.html](stock_web/static/index.html)) prompts the user to enter the close manually — which `PUT /api/taiex/today` persists and back-patches into existing per-stock caches.
...
**Pre-fetch automation** (separate launchd job, weekdays **07:00 + 16:30**): `tools/launchd/install_watchlist_refresh.sh` installs `com.user.claude-watchlist-refresh`, which runs `python3 -m tools.refresh_watchlist`. That script mirrors `/api/watchlist/refresh` (drop per-stock cache + re-fetch via the shared TWSE/T86/companies pipeline) and additionally calls `backtest.prefetch_us.fetch_ticker` for the 7 US tickers. No `claude` CLI involved → no token cost. The **07:00** run is pre-market warm-up (refreshes the prior completed day + overnight US closes, so the dashboard loads instantly at open). The **16:30** run is post-close: it lands right on the trading-day rollover (`TRADING_DAY_ROLLOVER_HOUR`/`_MINUTE`), so today's daily OHLCV / T86 / 融資借券 are published and the cache tag has advanced to today — this run pulls today's complete data. Log at `tools/launchd/watchlist-refresh.log` (gitignored).
