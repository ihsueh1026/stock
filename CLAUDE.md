...
**Pre-fetch automation** (separate launchd job, weekdays **07:00 + 17:00**): `tools/launchd/install_watchlist_refresh.sh` installs `com.user.claude-watchlist-refresh`, which runs `python3 -m tools.refresh_watchlist`. That script mirrors `/api/watchlist/refresh` (drop per-stock cache + re-fetch via the shared TWSE/T86/companies pipeline) and additionally calls `backtest.prefetch_us.fetch_ticker` for the 7 US tickers. No `claude` CLI involved → no token cost. The **07:00** run is pre-market warm-up (refreshes the prior completed day + overnight US closes, so the dashboard loads instantly at open). The **17:00** run is post-close: it lands right on the trading-day rollover (`TRADING_DAY_ROLLOVER_HOUR`), so today's daily OHLCV / T86 / 融資借券 are published and the cache tag has advanced to today — this run pulls today's complete data. Log at `tools/launchd/watchlist-refresh.log` (gitignored).
