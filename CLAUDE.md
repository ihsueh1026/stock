...
- **Sibling fetchers in [stock_web/](stock_web/)** — each module is a single-responsibility fetcher with its own daily-cached JSON output and a matching `/api/...` endpoint:
  ...
  - [eps_history_fetcher.py](stock_web/eps_history_fetcher.py) — multi-year quarterly EPS via MOPS ajax_t164sb04. **Q4 quirk**: t164sb04 returns standalone EPS for Q1-Q3 but **cumulative full-year** EPS for Q4 (because the Q4 filing is the annual report). The fetcher derives Q4 = annual − (Q1+Q2+Q3) explicitly. **Financial-holding fallback**: t164sb04 (一般業 quarterly 綜合損益表) returns nothing for 金融保險業 (金控/銀行/保險/證券) — they file under a different statement template. `_annual_fallback()` then pulls ANNUAL EPS from `fundamentals_fetcher` (t146sb05 簡明財報, which covers financial statements), returning `granularity:"annual"` with year-labelled periods (~3 years). The `/api/eps_history` endpoint skips the `eps_state` quarterly-accel badge for annual data, and the frontend retitles the panel 季→年度 EPS 趨勢 + relaxes the min-bars threshold 4→3.
...
`_trading_day()` in [stock_web/app.py](stock_web/app.py) returns the current trading date, walking back over weekends to the prior Friday. **It rolls over at 16:30** (`TRADING_DAY_ROLLOVER_HOUR`/`_MINUTE`): before 16:30 local, today's daily OHLCV + T86 (三大法人, ready ~16:00) aren't out yet, so the "current trading day" stays on the previous completed day; at 16:30 it advances to today. The `refresh_watchlist` launchd job's afternoon run (16:30) fires right after this rollover to pull today's close + 法人. **NB: 融資/融券/借券 publish much later — ~21:00–21:30 — so the 16:30 run can't get them; the next morning's 07:00 run picks up the prior trading day's margin/SBL** (`_refresh_margin_sbl` walks back to the last published day). All daily caches (`{code}_{YYYYMMDD}.json`, `taiex_{YYYYMMDD}.json`, `t86_{YYYYMMDD}.json`, `companies_{YYYYMMDD}.json`) share this same tag. If today's TAIEX close hasn't been published yet (e.g. fetched intraday or right after 13:30 close), `taiex` rows come back null and the frontend's `taiexBar` ([stock_web/static/index.html](stock_web/static/index.html)) prompts the user to enter the close manually — which `PUT /api/taiex/today` persists and back-patches into existing per-stock caches.
...
- `GET /api/broker_branch/{code}?days=20` — per-stock 券商分點 集中度 (main-force concentration). Reads the **semi-manual** accumulation log `stock_web/broker_branch_log.jsonl` (built by `stock_web/broker_branch_ingest.py`, see workflow below). Returns the latest (code,date) record — 集中度% (前15大主力淨買賣 ÷ 當日量), 主力淨/買均, 買家/賣家家數, top-15 buy/sell branches — plus a concentration trend over the last N ingested dates and a `state` badge (主力強吸貨 ≥+15% / 偏買 ≥+5% / 中性 / 偏賣 ≤−5% / 強出貨 ≤−15%). `available:false` if the code was never ingested (so the 券商分點 detail-page card hides cleanly). **No free automated source** — TWSE BSR is CAPTCHA-gated, FinMind 分點 is sponsor-only — so data only exists for codes the user has fed in. Read-only + mtime-cached; log is gitignored per-user runtime data. A shared `_broker_conclusion(code, ref_date)` helper surfaces a **conclusion chip** for the strong tier only (|集中度| ≥ 15%, and only if the latest ingest is ≤7 days older than ref_date): in the detail-page 提醒 strip (via `_compute_alerts`, appended without a `stat_key`) and in the homepage 今日觸發 panel (via a separate path in `watchlist_chips()` under `broker_concentration_buy`/`broker_concentration_sell`). Both paths are deliberately **forward-log-free** — the chip is observational, so it never enters the curated OOS dataset (`forward_log.jsonl`) the validated chips use.
...
Several log files live at `stock_web/*.jsonl` (all gitignored as per-user runtime data). They accumulate over time and are deliberately not automated, so the user controls when data refreshes.
...
### `stock_web/broker_branch_log.jsonl` — 券商分點 集中度 (semi-manual)

There is **no free automated source** for per-branch trading data: TWSE BSR is CAPTCHA-gated and FinMind 分點 is sponsor-only (history from 2021-06-30). So this is hand-fed. Two ingestion paths, both ending in `python3 -m stock_web.broker_branch_ingest`:

1. **Manual paste** — user drops one `{code}.csv` (or `.txt`) per stock into `stock_web/broker_input/` (raw 分點分價 "券商買賣股票成交價量資訊" export, Big5 or UTF-8). `_FORMAT.txt` documents the layout.
2. **User's BSR crawler** — `twse_web/twse_broker_scraper.py` (uses `ddddocr` to solve the BSR CAPTCHA) writes `{code}.csv` to `twse_web/output/`. **Claude must NOT run or schedule this crawler** — solving a CAPTCHA is out of scope; the user runs it themselves. Claude only does the plumbing around it: `twse_web/export_watchlist_codes.py` regenerates `watchlist_twse.txt` (上市-only codes for `-i`), and `broker_branch_ingest.py` reads `twse_web/output/*.csv` as a second source (auto-detects Big5 vs UTF-8-BOM).

`broker_branch_ingest.py` parses the 分點分價 detail (each row = broker × price, two records per line), aggregates per broker → 集中度 (前15大主力淨買賣 ÷ 當日量), top-15 buy/sell branches, 主力買均, 買家/賣家家數. Appends one record per (code, date) — idempotent dedup on (code, date), `--force` to override, `--date YYYY-MM-DD` (default = prev weekday; files carry no date so pass it explicitly when needed). Served to the detail-page 券商分點 card via `/api/broker_branch/{code}`. Intent: accumulate enough dates to backtest whether 集中度 carries independent edge (don't ship a study until the sample is meaningful).
...
| 「更新分點」/「整理分點」 | Run `python3 -m stock_web.broker_branch_ingest` (scans `stock_web/broker_input/` + `twse_web/output/`, auto-dedups). Confirm the data date with the user (files carry none) — pass `--date YYYY-MM-DD` if it's not the prev weekday. Report per-code 集中度 + main-force net. **Never run `twse_web/twse_broker_scraper.py` yourself — it solves a CAPTCHA; the user runs the crawl.** Regenerate the crawler's input list with `python3 twse_web/export_watchlist_codes.py` when the watchlist changed. |
| 「跑分點集中度分析」 | Read `stock_web/broker_branch_log.jsonl`, join with `forward_log.jsonl` on (code, date), bucket by 集中度 sign/magnitude, report forward alpha per bucket. If <3 months / thin sample, say "early — keep accumulating" rather than overselling. |
...
**Pre-fetch automation** (separate launchd job, weekdays **07:00 + 16:30**): `tools/launchd/install_watchlist_refresh.sh` installs `com.user.claude-watchlist-refresh`, which runs `python3 -m tools.refresh_watchlist`. That script mirrors `/api/watchlist/refresh` (drop per-stock cache + re-fetch via the shared TWSE/T86/companies pipeline) and additionally calls `backtest.prefetch_us.fetch_ticker` for the 7 US tickers. No `claude` CLI involved → no token cost. The **07:00** run is pre-market warm-up: refreshes the prior completed day + overnight US closes + the prior day's **融資/融券/借券** (which only publish ~21:00–21:30 the night before), so the dashboard loads instantly at open. The **16:30** run is post-close: it lands right on the trading-day rollover (`TRADING_DAY_ROLLOVER_HOUR`/`_MINUTE`), so today's daily OHLCV + T86 are published and the cache tag has advanced to today — this run pulls today's close + 法人 (margin/SBL aren't out yet, they come via the next 07:00). Log at `tools/launchd/watchlist-refresh.log` (gitignored).
